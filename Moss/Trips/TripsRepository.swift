...
@MainActor final class TripsRepository: ObservableObject {
    ...
    static func classifyCreationError(_ error: Error) -> TripCreationError { if let e=error as? TripCreationError{return e}; if let e=error as? PostgrestError { let code=e.code ?? ""; if code=="MS001" {return .quotaReached}; if code=="MS002" {return .subscriptionVerificationPending}; if ["28000","PGRST301","PGRST302","PGRST303"].contains(code){return .authenticationRequired}; if code=="42501" || code.hasPrefix("22") || code.hasPrefix("23"){return .serverValidation(e.message)}; return .unknown(e.message) }; if error is URLError{return .connectivity(error.localizedDescription)}; return .unknown(error.localizedDescription) }
